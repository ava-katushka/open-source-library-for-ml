...
def load_data_kaggle_MNIST(dataset_name='train.csv', pkl_name='training_set.pkl'):
    ...
    :return:
    """
    print '... loading data in kaggle_MNIST stylle'
    ...
    with open("semeion.data") as input_file:
        for line in input_file:
            ...
            for number in line.split():
                if j < 256:
                	a = float(number)
                    data[i].append(tuple(a))
                else:
                    if float(number) > 0.5:
                        label.append(tuple(j - 256))
                j += 1
            i += 1
    dset = [] * 2
    for i in range(1593):
        dset.append([])
        dset[i].append(label[i])
        for j in range(256):
            dset[i].append(data[i][j])
	
	dataset = numpy.asarray(dset)
