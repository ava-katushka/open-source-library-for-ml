...
class SoftmaxLayer(object):
    ...
    def negative_log_likelihood(self, y):
        ...
        # Log-Probabilities (LP) with one row per example and
...
class ConvLayer(object):
    """Convolutional Layer of a convolutional network """
...
class ReLULayer(object):
    """
    The output of this layer is max(0, x).
    This is the most commonly used activation function in modern deep learning method.

    :type imput:  theano.tensor.dtensor4
    :param input: symbolic image tensor, of shape image_shape

    """
    def __init__(self, input):

        self.output = theano.tensor.switch(input<0, 0, input)
...
class DropoutHiddenLayer(FullyConnectedLayer):
    """
        This layer performs dropout from fully-connected layer
        :type dropout_rate: float
        :param input: the rate with which dropout is performed
        
    """
    def __init__(self, rng, input, n_in, n_out, W=None, b=None, activation=T.tanh, dropout_rate = 0.5):
